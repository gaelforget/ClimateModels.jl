...
begin
	using ClimateModels, MITgcm, PlutoUI, Plots, CSV
...
begin
	...
	The [ClimateModels.jl](https://github.com/gaelforget/ClimateModels.jl) interface for the [MIT general circulation model (MITgcm)](https://mitgcm.readthedocs.io/en/latest/?badge=latest) is implemented in [MITgcm.jl](https://gaelforget.github.io/MITgcm.jl/dev/).
...
The **$(typeof(exps[iexp]))** type (a concrete type of the `AbstractModelConfig`, as is `ModelConfig`) is provided by `MITgcm.jl`. Doing this allows `setup`, `build`, `launch`, etc to readily dispatch to the appropriate methods which are also provided by `MITgcm.jl`. **A similar approach can be applied to any (?) other climate model.**
...
manifest_format = "2.0"
project_hash = "28648fee9c5a5a11e717db7f96adb69475031427"
