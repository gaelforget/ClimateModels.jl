...
using ClimateModels, Suppressor, Downloads

import ClimateModels: setup, build, AbstractModelConfig, read_IniFile
...
""" 
Base.@kwdef struct HectorConfig <: AbstractModelConfig
	model :: String = "Hector"
...
function build(x :: HectorConfig; exe="")
	if isempty(exe)
		pth0=pwd()
		pth=pathof(x)
...
function Hector_launch(x::HectorConfig)
    pth0=pwd()
    pth=pathof(x)
...
function calc_all_scenarios(MC)
	...
	store=fill(HectorConfig(),length(list))
	for ii in 1:length(list)
		tmp=HectorConfig(configuration=list[ii],folder=MC.folder,ID=MC.ID)
